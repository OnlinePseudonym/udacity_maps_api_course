...
// Create placemarkers array to use in multiple function to have control over
// the number of places that show.
var placeMarkers = [];
...
function initMap() {
    ...
    // Creat a searchbox in order to execute a places search
    var searchBox = new google.maps.places.SearchBox(
        document.getElementById('places-search'));
    // Bias the searchbox to within the bounds of the map.
    searchBox.setBounds(map.getBounds());

    // These are the real estate listings that will be shown to the
    // user. Normally we'd have these in a database instead.
    var locations = [
    ...
    // Style the markers a bit. This will be our listing marker
    // icon.
    var defaultIcon = makeMarkerIcon('0091ff');

    // Create a "highlighted location" marker color for when the
    // user mouses over the marker.
    ...
    for (var i = 0; i < locations.length; i++) {
        ...
        markers.push(marker);
        // Create an onclick event to open an infoWindow at each
        // marker.
        marker.addListener('click', function() {
            populateInfoWindow(this, largeInfoWindow);
        });
        // Two event listeners - one for mouseover, one for
        // mouseout, to change the colors back and forth.
    ...
    // Listen for the event fired when the user selcts a prediction and clicks
    // "go" more details for that place.
    document.getElementById('go-places').addEventListener('click',
        textSearchPlaces);

    // Add an event listener so that the polygon is captured, call the
    // serchWithinPolygon function. This will show the markers in the polygon,
...
// clicked. We'll only allow one infoWindow which will open at
// the marker that is clicked, and populate based on that
// markers position.
function populateInfoWindow(marker, infoWindow) {
    ...
    if (infoWindow.marker != marker) {
        ...
        // compute the position of the streetview image, then
        // calculate the heading, then get a panorama from that and
        // set the options.
        ...
        // Use streetview service to get the closest streetview
...
function showListings() {
    ...
    // Extend the boundaries of the map for each marker and display
    // the marker
...
// locates it, and then zooms into that area. This is so that the user can show
// all listings, then decide to focus on one area of the map.
function zoomToArea() {
    // Initialize the geocodert.
        ...
        geocoder.geocode(
            {
                ...
                if (status == google.maps.GeocoderStatus.OK) {
                    ...
                    window.alert('We could not find that location - try' +
                        ' entereing a more specific place.');
                }
            }
        );
    }
...
function searchWithinTime() {
    ...
    if (address == '') {
        ...
        // routes between all our markers, and the destination addresss entered
    ...
    for (var i = 0; i < origins.length; i++) {
        ...
        for (var j = 0; j < results.length; j++) {
            ...
            if (element.status === "OK") {
                ...
                // If we wanted to switch the funtion to show markers within a
                // user-entered DISTANCE, we would need the value for distance,
                ...
                if (duration <= maxDuration) {
                    ...
                    var infoWindow = new google.maps.InfoWindow({
                        content: durationText + ' away, ' + distanceText +
                            '<div><input type=\"button\" value=\"View Route\"' +
...
// markers within the calculated distance. This will display the route on the
// map.
...
function searchBoxPlaces(searchBox) {
    ...
    if (places.length == 0) {
        window.alert('We did not find any places matching that seach!');
    }
}

// This funciton fires when the user select "go" on the places search.
// It will do a nearby search using the entered querty string or place.
...
function getPlacesDetails(marker, infoWindow) {
    ...
    service.getDetails({
        ...
        if (status === google.maps.places.PlacesServiceStatus.OK) {
            ...
            infoWindow.open(map, marker);
            // Make sure the marker property is cleared if the infoWindow is
            // closed.
            infoWindow.addListener('closeclick', function() {
                infoWindow.marker = null;
            });
